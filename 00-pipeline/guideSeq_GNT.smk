...
#include: "Prepare_genomes.snakemake"
...
rule get_fasta_around_is:
    ...
    shell: """
        ...
        sort -k11n {input.bed} | bedtools groupby -g 11 -c 1,2,3,4,5,6,7,8 -o distinct,min,max,count_distinct,median,count_distinct,sum,sum |  awk 'BEGIN{{OFS="\\t"}}{{print $2,$3,$4,$1,$7,$6,$5,$8,$9}}' |             bedtools slop -i - -b {params.slop_size} -g {params.chr_length} > {output.cluster} 
        bedtools getfasta -name -fi {params.fasta} -bed {output.cluster} > {output.fa}
        """ 


rule get_stats_fq:
    input: rules.demultiplex_library.output.R1, rules.trim_ODN.output.R1, rules.trim_reads.output.R1, rules.filter_reads.output.R1
