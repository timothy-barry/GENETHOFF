...
rule call_IS:
    ...
    shell: """
        ...
        awk '$6>{params.minReadsPerFragment}' {output.frag} |  bedtools groupby -g 1,2,3,5,8 -c 4,6,4,6,7  -o count_distinct,sum,collapse,collapse,median  | awk 'BEGIN{{OFS="\\t";FS="\\t"}} {{print $1,$2,$3,"ID_"NR,$10,$4,$6,$7,$8,$9,$5}}' > {output.collapse}
        """
...
rule report_data:
    ...
    shell : """
        Rscript ../multiple_alignments.R {input.fasta} {input.cluster} {input.bed} {params.gRNA_seq} {params.gRNA_name} {output}
        """
